...
class ProReportLinkScreen extends StatelessWidget {
  ProReportLinkScreen({super.key});

  final _controller = Get.find<ExaminationReportController>();
  final GlobalKey _qrKey = GlobalKey();

  Future<void> qrCodePreview() async {
    _controller.previewProReportQrPdf();
  }

  Future<void> printQrCode() async {
    String? printMessage = await _controller.printProReportQr();
    if (printMessage != null) Get.snackbar("列印失敗", printMessage);
  }

  Future<void> _shareQrCoder() async {
    await _controller.shareProfessionalReportLink();
  }

  Future<void> _copyLink() async {
    _controller.copyLinkToClipboard(_controller.professionalReportLink);
    Get.snackbar("成功", "成功將連結複製至剪貼簿!");
  }

  Future<void> _openLink() async {
    final Uri _url = Uri.parse(_controller.professionalReportLink);
    if (!await launchUrl(_url)) {
      Get.snackbar("失敗", "無法開啟網址連結");
      throw Exception("Could not launch ${_controller.professionalReportLink}");
    }
  }

  @override
  Widget build(BuildContext context) {
    bool isLoading = _controller.reportCaseId.value.isEmpty ||
        _controller.phoneNumber.value.isEmpty;

    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text("專業報告"),
      ),
      body: Column(
        children: [
          const KampoTitle(title: "專業報告 QR Code"),
          Expanded(
            child: Obx(
                  () => isLoading
                  ? const SliverLoading()
                        ...
                        Container(
                          ...
                          child: RepaintBoundary(
                            ...
                            child: BarcodeWidget(
                              ...
                              data: _controller.professionalReportLink,
                            ),
                          ),
                        ),
                        const Divider(height: 20, color: Colors.transparent),
