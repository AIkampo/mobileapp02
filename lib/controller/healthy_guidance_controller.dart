...
class HealthyGuidanceController extends GetxController {
  ...
  Future<void> fetchDietData(caseId) async {
    ...
    try {
      final response = await Dio().get(
        "https://www.oberonhc.com/api/Score/23/${caseId}",
      ...
      if (response.data['success']) {
        ...
        if (dietList.length >= 20) {
          ...
          disrecommendFoods
              .addAll(dietList.reversed.toList().getRange(0, dietList.length));
        }
  ...
  Future<void> fetchNutrientsData(caseId) async {
    print("****** fetchNutrientsData(caseId) *****");
    isNutrientsDataLoading.value = true;
    nutrientsList.clear();
    try {
      final response = await Dio().get(
        "https://www.oberonhc.com/api/Score/162/$caseId",
        options: Options(headers: OrberonAPI.headers),
      );
      if (response.data['success']) {
        List tempList = response.data['data'].toList();
        for (var e
            in tempList.length >= 10 ? tempList.getRange(0, 10) : tempList) {
  ...
  Future<void> fetchAcupunctureData(caseId) async {
    print("****** fetchAcupunctureData(caseId) *****");
    isAcupunctureDataLoading.value = true;
    try {
      final response = await Dio().get(
        "https://www.oberonhc.com/api/Score/156/$caseId",
        options: Options(headers: OrberonAPI.headers),
      );
      if (response.data['success']) {
        List tempList = response.data['data'].toList();
  ...
  Future<void> fetchGemData(caseId) async {
    ...
    try {
      final response = await Dio().get(
        "https://www.oberonhc.com/api/Score/25/$caseId",
        options: Options(headers: OrberonAPI.headers),
      );
      if (response.data['success']) {
        List tempList = response.data['data'].toList();
